...
class BJ_Hand(cards.Hand):
    ...
    def total(self):
        ...
        for card in self.cards:
            if not card.value:
                return
        # суммируются очки считая каждый туз за 1 очко
        t = 0
        for card in self.cards:
            t += card.value
        # определяет есть ли туз на руках у игрока
        contains_ace = False
        for card in self.cards:
            if card.value == BJ_Card.ACE_VALUE:
                contains_ace = True
        # если на руках есть туз и сумма очков не аревышает 11 будем считать туз за 11 очков
        if contains_ace and t <=11:
            # прибавляем только 10 потому что 1 уже пошло в общую сумму
            t+=10
        return t
    def is_busted(self):
...
class BJ_Game(object):
    ...
    def play(self):
        ...
        self.dealer.flip_first_card()# первая из карт сданных дилллеру
        ...
        if not self.still_playing:
            ...
            if self.dealer.is_busted():
                ...
                # сравниваем суммы очков у дилера и у игроков в игре
                for player in self.still_playing:
